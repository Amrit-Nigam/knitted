...
public enum CuffRenderer {
    public static func ribbingSwatch(palette: YarnPalette, stitch: StitchSize, scale: CGFloat) -> Swatch? {
        ...
        return SwatchRenderer.render(pattern: .ribbing, palette: ribbingPalette, stitch: stitch, scale: scale)
    }
    ...
    /// `depth` is the band's full thickness (border width plus any overlap under the window).
