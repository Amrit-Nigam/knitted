...
public struct BorderStyle: Equatable, Sendable {
    ...
    /// Corner radius of the rectangle being framed.
    public var cornerRadius: CGFloat
    /// How far the fabric reaches under the framed content, so no sliver of background shows
    /// at the seam.
    ...
    public init(borderWidth: CGFloat = 14, cornerRadius: CGFloat = 12, holeOverlap: CGFloat = 0,
...
/// Composes swatches into a knitted ring around a rounded rectangle. Expects a y-down context
/// in points whose `bounds` is the rectangle outset by `borderWidth`.
///
...
public enum BorderPainter {
    ...
    struct Geometry {
        let bounds: CGRect
        let depth: CGFloat
        /// How deep the mitred edge strips reach: past the fabric and the rounded
        ...
        init(bounds: CGRect, style: BorderStyle) {
            ...
            reach = bw + max(style.cornerRadius, 0) + style.flare

            let target = bounds.insetBy(dx: bw, dy: bw)
            let hole = target.insetBy(dx: style.holeOverlap, dy: style.holeOverlap)
            let holePath = Self.roundedRect(hole, radius: style.cornerRadius - style.holeOverlap)
            let outer = Self.roundedRect(bounds, radius: style.cornerRadius + bw)
            let flared = Self.roundedRect(bounds.insetBy(dx: style.flare, dy: style.flare),
                                          radius: style.cornerRadius + bw - style.flare)
